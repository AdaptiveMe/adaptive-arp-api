...
public class CSharpGenerator extends GeneratorBase {
    ...
    protected void startCustomClass(String className, Class clazz, JavaClass javaClass, boolean implementation) {
        if (implementation) {
            currentFile = new File(getOutputRootDirectory(), camelCase(clazz.getPackage()).substring(0, camelCase(clazz.getPackage()).lastIndexOf('.')) + ".Impl" + File.separator + className + ".cs");

        } else {
            currentFile = new File(getOutputRootDirectory(), camelCase(clazz.getPackage()) + File.separator + className + ".cs");
        }
        currentFile.mkdirs();
