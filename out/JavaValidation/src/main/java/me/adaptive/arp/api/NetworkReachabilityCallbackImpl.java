...
public class NetworkReachabilityCallbackImpl extends BaseCallbackImpl implements INetworkReachabilityCallback {
     ...
     */
     public void onError(INetworkReachabilityCallbackError error) {
          AppRegistryBridge.getInstance().getPlatformContextWeb().executeJavaScript("Adaptive.handleNetworkReachabilityCallbackError( '"+getId()+"', JSON.parse(" + getJSONAPI().toJson(error) +") )");
     }
     ...
     */
     public void onResult(boolean reachable) {
          AppRegistryBridge.getInstance().getPlatformContextWeb().executeJavaScript("Adaptive.handleNetworkReachabilityCallbackResult( '"+getId()+"', JSON.parse(" + getJSONAPI().toJson(reachable) +") )");
     }
     ...
     */
     public void onWarning(boolean reachable, INetworkReachabilityCallbackWarning warning) {
          AppRegistryBridge.getInstance().getPlatformContextWeb().executeJavaScript("Adaptive.handleNetworkReachabilityCallbackWarning( '"+getId()+"', JSON.parse(" + getJSONAPI().toJson(reachable) +"), JSON.parse(" + getJSONAPI().toJson(warning) +") )");
     }
